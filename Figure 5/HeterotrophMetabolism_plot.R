...
a<-read.csv("/Users/andrewabraham/Dropbox/Madingley & Megabiota/Manuscript_Scripts/HeterotrophMetabolism_MassBins_MEAN.csv", stringsAsFactors=F)

tiff('HeterotrophicMetabolism_MassBins_plot.tiff', units="in",width=5, height=5, res=500)

q1<-ggplot() + theme_bw() + 
  scale_y_continuous(name="Heterotroph Metabolism (EJ/day)", breaks=c(0,0.2,0.4),
                     labels=c("0.0","0.2","0.4"),limits=c(0,0.4)) +
...
b<-read.csv("/Users/andrewabraham/Dropbox/Madingley & Megabiota/Manuscript_Scripts/HeterotrophMetabolism_EndEct_MEAN.csv", stringsAsFactors=F)

q2<-ggplot() + theme_bw() + xlab("") +
  scale_y_continuous(name="Global Heterotroph Metabolism \n(EJ/day)", breaks=c(0,2.1,4.2),
                     labels=c("0","2.1","4.2"),limits=c(0,4.2)) + 
