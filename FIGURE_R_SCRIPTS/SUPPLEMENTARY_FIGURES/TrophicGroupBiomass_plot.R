...
c<-read.csv("/Users/andrewabraham/Dropbox/Madingley & Megabiota/Manuscript_Scripts/Global_TrophicGroup_NutrientDiffusivity_MEAN.csv", stringsAsFactors=F)

tiff('TrophicGroup_NutDiff.tiff', units="in",width=8, height=5, res=500)

ggplot(data=c,aes(x=as.numeric(c[,2]),y=c[,3]/10^7,fill=c[,1])) +
  geom_bar(position="dodge", stat="identity",width = 0.7) + 
  scale_y_continuous(name=expression(Nutrient~Diffusivity~(10^6~km^2/day)), breaks=c(0,1.5,3),
                     labels=c("0","1.5","3.0"),limits=c(0,3)) +
  scale_x_continuous(name="\nTrophic Group", breaks=c(1,2,3,4,5,6),
                     labels=c("Endothermic \nHerbivore","Endothermic \nCarnivore","Endothermic \nOmnivore",
                              "Ectothermic \nHerbivore","Ectothermic \nCarnivore","Ectothermic \nOmnivore"),
                     limits=c(0.5,6.5)) +
