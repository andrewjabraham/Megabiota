...
#Load ensemble means
a<-read.csv("/Users/andrewabraham/Dropbox/Madingley & Megabiota/Manuscript_Scripts/Autotroph_MEAN.csv", stringsAsFactors=F)

#Load world boarders
wmap<-readOGR(dsn="/Users/andrewabraham/Dropbox/Madingley & Megabiota/ne_110m_land", layer="ne_110m_land")
wmap<-fortify(wmap)

tiff('Pleistocene_AB_map.tiff', units="in",width=5, height=5, res=500)
...
ggplot() + coord_equal() + 
  ...
  scale_fill_distiller(palette ="Spectral",na.value=NA,
                       breaks=c(10,1500000,3000000),
                       limits=c(0,3000000),
                       labels=c("0e+00","1.5e+06","3e+06"),
                       guide=guide_colourbar(title="Autotroph Biomass \n(kg/km2)"))
...
tiff('Sim1000_AB_map.tiff', units="in",width=5, height=5, res=500)
...
ggplot() + coord_equal() +  
  ...
  scale_fill_distiller(palette ="PuOr",na.value=NA,
                       breaks=c(-1700000,0,1700000),
                       limits=c(-1700000,1700000),
                       labels=c("-1.7e+06","0","1.7e+06"),
                       guide=guide_colourbar(title="Difference (kg/km2)\n "))

dev.off()


tiff('Sim100_AB_map.tiff', units="in",width=5, height=5, res=500)
